...
wire right_active;
assign right_active = h_bar & (x < right_dist);
assign right_dist_img = right_active ? 24'h03f8fc : crosshairs_img;

// Switch output pixels depending on mode switch
// Don't modify the start-of-packet word - it's a packet discriptor
// Don't modify data in non-video packets
assign {red_out, green_out, blue_out} = (mode & ~sop & packet_video) ? right_dist_img : {red,green,blue};
...
reg found_edge;
wire h_bar; assign h_bar = (y == 359);
wire r_half; assign r_half = (x > 320);

always @(posedge clk) begin
	if (~white_detect & in_valid & h_bar & r_half & ~found_edge) begin
		right_coord <= x;
	end
	else if (white_detect & in_valid & h_bar & r_half & ~found_edge) begin
		found_edge <= 1'b1;
	end
